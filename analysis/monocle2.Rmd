...
We will use `SingleR`, which is also on Bioconductor 3.10 to annotate cell types.
...
BiocManager::install(c("DropletUtils", "monocle", "SingleR", "BUSpaRse"))
```
...
```{bash, eval = FALSE}
cd ./output/neuron10k
kb count -i ../mm_tr_index97.idx -g ../tr2g_mm97.tsv -x 10xv3 -o kb \
...
Read the next section for instructions of going from fastq files to the matrices directly with `kallisto` and `bustools`. This is helpful to modularize the workflow, such as when you want to generate a regular gene count matrix and a TCC matrix for the same data, without having to make the `bus` file over and over again. The matrices for this analysis were generated in the section below.
...
```{r knee}
knee_plot(list(neuron_10k = bc_rank)) +
  coord_flip()
```
