...
Here I need to make sure that all transcripts in the capture list are in the dataset. Otherwise the output of `bustools capture` will be wrong. I hope that this will be fixed soon or will get a helpful error message.
```{r}
tr2g <- sort_tr2g(read_tsv("./output/neuron10k_collapse/tr2g.tsv", 
                           col_names = c("transcript", "gene")), 
                  kallisto_out_path = "./output/neuron10k_collapse")
writeLines(tr2g$transcript[str_detect(tr2g$transcript, "-I", negate = TRUE)],
           con = "./output/neuron10k_collapse/cDNA_tx_to_capture.txt")
writeLines(tr2g$transcript[str_detect(tr2g$transcript, "-I")],
           con = "./output/neuron10k_collapse/introns_tx_to_capture.txt")
```

```{r, include=FALSE}
do_copy <- !file.exists("./data/whitelist_v3.txt")
do_bustools <- !file.exists("./output/neuron10k_collapse/output.correct.sort.bus")
do_count <- !file.exists("./output/neuron10k_collapse/spliced.mtx")
```
...
In the most recent development version of Seurat, `RunUMAP` can use the R package `uwot` as the backend, thus obliterating calling Python UMAP through `reticulate`. On servers, `reticulate` does not work due to [this issue](https://github.com/rstudio/reticulate/issues/428).
...
This presents a much more complicated picture. The cells labeled qNSCs are at the very bottom, and there seem to be two paths for the early stage cells to go towards neurons. The more mature neurons also seem to be changing a lot. This step is computationally expensive; in subsequent calls to `show.velocity.on.embedding.cor` for the same dimension reduction, the expensive part can be bypassed by supplying the output of the first call.
```{r v2}
show.velocity.on.embedding.cor(emb = Embeddings(seu, "umap"),
                               vel = Tool(seu, slot = "RunVelocity"),
                               n.cores = 50, show.grid.flow = TRUE,
...
qNSCs and closely related astrocytes are in the cluster at the very bottom of this plot. This picture is mostly consistent with going from qNSCs to neurons, though manual cell type annotation would be helpful.
