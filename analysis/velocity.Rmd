...
```{r, include=FALSE}
do_untar <- !dir.exists("./data/neuron_10k_v3_fastqs")
do_index <- !file.exists("./output/mm_cDNA_introns_94_collapse.idx")
...
```{bash, eval=do_index}
# Intron index
kallisto index -i ./output/mm_cDNA_introns_94_collapse.idx ./output/neuron10k_collapse/cDNA_introns.fa
```
...
kallisto bus -i ../../output/mm_cDNA_introns_94_collapse.idx \
...
There's only one cell labeled ependymal by `SingleR`. How does number of UMI counts relate to number of genes detected? How does number of UMI counts in the spliced matrix relate to the number of gene detected in the unspliced matrix?
```{r}
...
```{r, eval=FALSE}
UMAPPlot(seu, pt.size = 0.5, group.by = "cell_type", label = TRUE, repel = TRUE) +
  scale_color_brewer(type = "qual", palette = "Set2")
```
...
It seems that something interesting is happening in cluster 5. 
