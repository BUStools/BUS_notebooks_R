...
# Seurat:::SpecificDimPlot. I still want to display the standard way though
...
UMAPPlot(seu, pt.size = 0.5)
```

```{r umap_clust, include=FALSE}
seu <- FindNeighbors(seu, verbose = FALSE) %>% 
  FindClusters(algorithm = 4, resolution = 1) # Leiden clustering
...
cell_colors_clust <- cell_pal(seu$SCT_snn_res.1, hue_pal())
```
...
model <- rand_forest(mtry = 40, trees = 1000, min_n = 15) %>%
  ...
  fit(pseudotime5 ~ ., data = dat_train)
```
...
```{r}
summary(dat_use_df$pseudotime5)
```
...
```{r genes, fig.width=6, fig.height=8}
par(mfrow = c(nr, nc))
